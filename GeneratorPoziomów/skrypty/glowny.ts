...
const C_WYSOKOSC: number = 400;
const C_SZEROKOSC: number = 800;
...
let KOLOR_TLA: string = '#dff0d8';
let PUNKT_STARTOWY: Kolo | null = null;
let PUNKT_DOCELOWY: Kolo | null = null;
let CHECK_POINTY: Kolo[] = [];
...
const OFFSET: Punkt = new Punkt(0, 0);
...
function wcisniecieKlawisza(klawisz: string): void {
    ...
    if (klawisz.toUpperCase() === 'G') GRID = !GRID;

    //Start point, End point, Checkpoint
    if (klawisz.toUpperCase() === 'S') PUNKT_STARTOWY = new Kolo(5, new Punkt(MYSZ.x, MYSZ.y), '#ff4c00');
    if (klawisz.toUpperCase() === 'E') PUNKT_DOCELOWY = new Kolo(5, new Punkt(MYSZ.x, MYSZ.y), '#fffa00');
    if (klawisz.toUpperCase() === 'C') CHECK_POINTY.push(new Kolo(5, new Punkt(MYSZ.x, MYSZ.y), '#42ce23'));
...
function animacja(): void {
    ...
    //Ruch kamery
    if (KLAWIATURA.StrzalkaLewo) OFFSET.x += 3;
    if (KLAWIATURA.StrzalkaPrawo) OFFSET.x -= 3;
    if (KLAWIATURA.StrzalkaGora) OFFSET.y += 3;
    if (KLAWIATURA.StrzalkaDol) OFFSET.y -= 3;
    CTX.save();
    ...
    if (PUNKT_STARTOWY !== null) PUNKT_STARTOWY.rysuj(CTX);
    if (PUNKT_DOCELOWY !== null) PUNKT_DOCELOWY.rysuj(CTX);
