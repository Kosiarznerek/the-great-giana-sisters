class Kolizja {
     ...
     * @returns {number}
     */
    public static WysokoscProstokatProstokatNPM(obj: Prostokat, przeszkody: Prostokat[]): number {
        const wyniki: number[] = [];

        przeszkody
            .filter((value =>
                    obj.pozycja.y + obj.wysokosc <= value.pozycja.y &&//wszystkie podemną
                    (//wszystkie pomiędy mną
            ...
            .forEach((value => {//Oblicz odleglości
     ...
     * @returns {boolean}
     */
    public static KoloProstokat(kolo: Kolo, prostokat: Prostokat): boolean {
        ...
        if (distX > (prostokat.szerokosc / 2 + kolo.promien)) return false;
        if (distY > (prostokat.wysokosc / 2 + kolo.promien)) return false;

        if (distX <= (prostokat.szerokosc / 2)) return true;
        if (distY <= (prostokat.wysokosc / 2)) return true;
     ...
     */
    public static AktualizujKoloUnikajac(obj: Kolo, wekt: Wektor, przeszkody: Prostokat[]): void {
        //Prostokat reprezentujacy kolo
        const p = new Prostokat(
            new Wektor(obj.pozycja.x - obj.promien, obj.pozycja.y - obj.promien),
            obj.promien * 2,
            obj.promien * 2,
        );
     ...
     * @returns {boolean}
     */
    public static ProstokatNaProstokacie(obj: Prostokat, przeszkoda: Prostokat): boolean {
        return (
            obj.pozycja.y + obj.wysokosc === przeszkoda.pozycja.y &&
            (
