 ...
 */
function ZaladujGrafike(obrazek: HTMLImageElement, sciezka: string): Promise<HTMLImageElement> {
 ...
 * @returns {Promise<HTMLImageElement>[]}
 */
function PrzygotujGrafiki(obj = GRAFIKI, stack = '', array: Promise<HTMLImageElement>[] = []): Promise<HTMLImageElement>[] {
    for (let property in obj) {
        if (!obj.hasOwnProperty(property)) throw new Error('Nieoczekiwany błąd');
