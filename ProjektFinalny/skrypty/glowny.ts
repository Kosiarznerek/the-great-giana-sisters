...
//Canvas
const C_SZEROKOSC: number = 800; //szerokosc canvasu
const C_WYSOKOSC: number = 400; //wysokosc canvasu
let CTX: CanvasRenderingContext2D; //ctx canvasu

//RequestAnimationFrameID
let REQ_ID: number;

//Dane bierzacego poziomu
let BIERZ_POZ: Poziom;

//Kontroler rozgrywki
const KONTROLER: KontrolerRozgrywki = new KontrolerRozgrywki(
    new Wektor(0, 0),
    40,
    C_SZEROKOSC,
    12
 ...
 */
function klawiszDol(klawisz: string): void {
    switch (klawisz.toUpperCase()) {
        ...
        case ' ':
            ...
            break;
        case 'P':
            if (!KONTROLER.uprawnienia.rzutKulka) return;
            BIERZ_POZ.gracz.rzucKulke(
                new Array()//przeszkody od jakich odbija się pilka
                ...
                new Array()//zywi przeciwnicy do jakich celuje gracz
                    .concat(BIERZ_POZ.przeciwnicy.chodzacy.zywi)
                    .concat(BIERZ_POZ.przeciwnicy.skaczacy)
                    .concat(BIERZ_POZ.przeciwnicy.pajaki)
                    .concat(BIERZ_POZ.przeciwnicy.smoki),
            ...
            break;
        case 'B':
            if (!KONTROLER.uprawnienia.detonacjaBomby) return;
            BIERZ_POZ.gracz.detonujBombe(BIERZ_POZ.przeciwnicy.chodzacy.zywi).forEach(value => value.zabij());
            if (!KONTROLER.uprawnienia.wielokrotnaDetonacja) KONTROLER.uprawnienia.detonacjaBomby = false;
            detonator = 50;
            break;
    }
}
 ...
 * @param {string} klawisz
 */
function klawiszGora(klawisz: string): void {
    switch (klawisz.toUpperCase()) {
        case 'A':
...
async function petlaAnimacji() {
    //Tło
    if (detonator <= 0) CTX.fillStyle = BIERZ_POZ.kolorTla;
    else if (detonator > 0) {
        if (detonator % 6 === 0) CTX.fillStyle = '#000000';
    ...
    BIERZ_POZ.gracz.uderzylOdSpodu(BIERZ_POZ.przeszkody.niespodzianki).forEach(value => {
        ...
        let nagroda: 'DIAMENCIK' | UciekajacaNagroda | null = value.zamienWStal();
        if (nagroda === 'DIAMENCIK') {
            AudioKontroler.Odtworz(AUDIO.uderzenie);
            KONTROLER.statystyki.bonusy++;
        }
        if (nagroda instanceof UciekajacaNagroda) BIERZ_POZ.uciekajace_nagrody.push(nagroda);
    });
    ...
    if (BIERZ_POZ.przeciwnicy.chodzacy.zywi.filter(value => BIERZ_POZ.gracz.wszedlNa(value)).length > 0)
        if (!KONTROLER.uprawnienia.niesmiertelnosc) BIERZ_POZ.gracz.zabij(() => {
    ...
    BIERZ_POZ.uciekajace_nagrody = BIERZ_POZ.uciekajace_nagrody.filter(value => {
        //Jeżeli jeszcze tego nie zebrał to zostaw to na mapie
        if (!BIERZ_POZ.gracz.wszedlNa(value)) return true;
