class Gracz extends Prostokat {
    ...
    public ruch: { lewo: boolean, prawo: boolean, skok: boolean };
    private _przesuniecie: Wektor;

    private _startowaSkoku: number | null;
    private _poprzedniaPozycja: Wektor;

    private _kierunekPatrzenia: Wektor;

    private _animacje: {
        normalny: {
            lewoChodzenie: AnimatorPoklatkowy,
            prawoChodzenie: AnimatorPoklatkowy
        },
        paralityk: {
            lewoChodzenie: AnimatorPoklatkowy,
            prawoChodzenie: AnimatorPoklatkowy
        },
        transformacja: AnimatorPoklatkowy
    };
    private _zadanyTryb: 'normalny' | 'paralityk';
    private _kulka: Kulka | null;
    private _zabity: boolean;
     ...
     */
    constructor(pozycja: Wektor, wysokosc: number, szerokosc: number, tryb: 'normalny' | 'paralityk') {
        super(pozycja, wysokosc, szerokosc, GRAFIKI.gracz.paralityk.prawo.spoczynek);

        this.ruch = {//Od tych zmiennych zależy czy gracz ma czy nie ma iść w danym kierunku
            lewo: false,
            prawo: false,
            skok: false
        };
    ...
    private _obecnyTryb: 'normalny' | 'paralityk';

    /**
     * Zwraca obecny tryb w jakim jest gracz
     * @returns {"normalny" | "paralityk"}
     */
    get obecnyTryb(): 'normalny' | 'paralityk' {
    ...
    public aktualizujPozycje(przeszkody: Prostokat[]): void {
        ...
        //Kontrola (prawo-lewo)
        if (this.ruch.lewo && this._przesuniecie.x > -4) this._przesuniecie.x -= 0.32;
        if (this.ruch.prawo && this._przesuniecie.x < 4) this._przesuniecie.x += 0.32;

        //Spowolnienie (prawo-lewo)
        if (this._przesuniecie.x > 0) this._przesuniecie.x -= 0.08;
        if (this._przesuniecie.x < 0) this._przesuniecie.x += 0.08;

        //Jeżeli prawo-lewo w zakresie (0,0.1) to ustaw ja na 0
        if (Math.abs(this._przesuniecie.x) > 0 && Math.abs(this._przesuniecie.x) < 0.1) this._przesuniecie.x = 0;

        //Skok
        if (this.ruch.skok && this._startowaSkoku === null && wysokosc !== 0) this.ruch.skok = false;//Ktoś próbował skoczyć nie będąc na podłodze
        if (this.ruch.skok && this._startowaSkoku === null) this._startowaSkoku = 0;//Rozpoczęcie skoku
        if (this.ruch.skok && this._przesuniecie.y >= -10 && this._startowaSkoku !== null && this._startowaSkoku < 100) this._przesuniecie.y -= 5;
        if (this._startowaSkoku !== null && this._przesuniecie.y < 0) this._startowaSkoku += Math.abs(this._przesuniecie.y);//obliczanie wysokości

        //Aktualizacja poprzedniej pozycji
        this._poprzedniaPozycja = this.pozycja.klonuj();

        //Grawitacja
        if (wysokosc > 0) this._przesuniecie.y += 1.5;
        if (wysokosc === 0 && this._startowaSkoku === null) this._przesuniecie.y = 0;

        //Aktualizacja pozycji
        Kolizja.AktualizujProstokatUnikajac(this, this._przesuniecie, przeszkody);

        //Skok zakonczony normalnie
        if (this._startowaSkoku !== null && wysokosc === 0 && this._startowaSkoku != Math.abs(this._przesuniecie.y)) this._startowaSkoku = null;
        ...
        if (this._poprzedniaPozycja.x === this.pozycja.x) this._przesuniecie.x = 0;
    }
    ...
    public aktualizujAnimacje(): void {
        ...
        if (this._obecnyTryb !== this._zadanyTryb) {
            let grafika: HTMLImageElement | null = this._animacje.transformacja.nastepnaKlatka();
            if (grafika !== null) this.grafika = grafika;
            //Transformacja zakonczona
            else {
        ...
        if (this.ruch.prawo) this._kierunekPatrzenia.x = 1;
        if (this.ruch.lewo) this._kierunekPatrzenia.x = -1;
        ...
        if (this._przesuniecie.x > 0 && this._przesuniecie.y === 0) {
            let grafika: HTMLImageElement | null = this._animacje[this._obecnyTryb].prawoChodzenie.nastepnaKlatka();
            if (grafika !== null) this.grafika = grafika;
            else {//Animacja sie zakończyła, więc urucham ponownie
        ...
        } else {//Nie idzie więc ustawiam animacje na początek
        ...
        //Gracz idzie w lewo
        if (this._przesuniecie.x < 0 && this._przesuniecie.y === 0) {
            let grafika: HTMLImageElement | null = this._animacje[this._obecnyTryb].lewoChodzenie.nastepnaKlatka();
            if (grafika !== null) this.grafika = grafika;
            else {//Animacja sie zakończyła, więc urucham ponownie
     ...
     */
    public uderzylOdSpodu<T extends Prostokat>(przeszkody: T[]): T[] {
     ...
     * @returns {boolean}
     */
    public wszedlNa<T extends Prostokat>(przedmiot: T): boolean {
        //Musi na niego wejść 1/4 ciała
        let poz: Wektor = new Wektor(this.pozycja.x + this.szerokosc / 4, this.pozycja.y + this.wysokosc / 4);
        let gracz: Prostokat = new Prostokat(poz, this.wysokosc / 2, this.szerokosc / 2);
     ...
     */
    public znajdujeSieNa<T extends Prostokat>(przeszkody: T[]): T[] {
     ...
     */
    public wskoczylNa<T extends Prostokat>(przeszkody: T[]): T[] {
        //Przesuwam gracza 'idealnie'
        let poz: Wektor = new Wektor(this.pozycja.x + this.szerokosc / 4, this.pozycja.y);
        let gracz: Prostokat = new Prostokat(poz, this.wysokosc, this.szerokosc / 2);
     ...
     */
    public zniszczKulke(): void {
        if (this._kulka) this._kulka.zniszcz();
    }
     ...
     */
    public rzucKulke(przeszkody: Prostokat[], przeciwnicy: Prostokat[]): void {
        //Kulka jest już w locie więc nie żucam 2 raz
        if (this._kulka !== null) return;

        //Kierunek w jakim bede rzucal
        let kierunekRzutu: Wektor;

        //Przeciwnik do ktorego mam najblizej
        let najblizszy: Prostokat = przeciwnicy.reduce((previousValue: Prostokat, currentValue: Prostokat): Prostokat => {
        ...
        const posStartowa: Wektor = this.pozycja.klonuj();
        if (kierunekRzutu.x > 0) posStartowa.x += this.szerokosc + Kulka.Promien;//po jego prawej
        else posStartowa.x -= Kulka.Promien;//po jego lewej
     ...
     */
    public rysujKulke(ctx: CanvasRenderingContext2D): void {
        if (this._kulka !== null) this._kulka.rysuj(ctx);
    }
     ...
     */
    public aktualizujKulke(ekran: Prostokat | undefined): void {
        if (this._kulka === null) return;
        ...
        if (Kolizja.KoloProstokat(this._kulka, this)) this._kulka = null;

        //Jeżeli kulka jest zniszczona -> moge rzucać jeszcze raz
        if (this._kulka !== null && this._kulka.calkowicieZniszczona) this._kulka = null;

        //Jeżeli nie ma 'ograniczen' ekranowych
        if (ekran === undefined) return;

        //Jeżeli kulka wyleci po za ekran to mogę rzucać jeszcze raz
        if (this._kulka && !Kolizja.KoloProstokat(this._kulka, ekran)) this._kulka = null;
    }
     ...
     */
    public trafilKulka<T extends Prostokat>(przedmioty: T[]): T[] {
        //Jeżeli kulka jest zniszczona to nie mogła nic trafić
        if (this._kulka !== null && this._kulka.zniszczona) return [];

        //Sprawdzam co trafiło
        return przedmioty.filter(value => {
            if (this._kulka !== null && !this._kulka.calkowicieZniszczona) return this._kulka.uderzyla(value);
            else return false;
        });
    }
     ...
     */
    public transformuj(tryb: 'normalny' | 'paralityk'): void {
        //Jeżeli tryb już jest wybrany to nic nie tobie
        if (this._obecnyTryb === tryb) return;

        //Jeżeli postać obecnie jest w trybie tranformacji to nic nie robie
        if (this._obecnyTryb !== this._zadanyTryb) return;
     ...
     */
    public detonujBombe<T extends Prostokat>(przeciwincy: T[]): T[] {
     ...
     */
    public zabij(callback: Function | undefined): void {
        //Jeżeli już jest zabity
        if (this._zabity) return;
