class Niespodzianka extends Prostokat {
    ...
    private _zamienionaWStal: boolean;
    private _doZamianyWStal: boolean;

    private _animacje: {
        glowna: AnimatorPoklatkowy,
        mruganie: AnimatorPoklatkowy
    };

    private _zawartosc: UCIEKAJACE_NAGRODY | 'DIAMENCIK';
    private _diamencik: null | Diamencik;
     ...
     */
    constructor(pozycja: Wektor, wysokosc: number, szerokosc: number, zawartosc: UCIEKAJACE_NAGRODY | 'DIAMENCIK') {
    ...
    public rysuj(ctx: CanvasRenderingContext2D): void {
        if (this._diamencik !== null) Prostokat.Rysuj(this._diamencik, ctx);
    ...
    public aktualizujGrafike(): void {
        ...
        if (this._zamienionaWStal) this.grafika = GRAFIKI.przeszkody.statyczne.stalowyKlocek;
        ...
        if (!this._zamienionaWStal && !this._doZamianyWStal) {
            let grafika: HTMLImageElement | null = this._animacje.glowna.nastepnaKlatka();
            if (grafika !== null) this.grafika = grafika;
            else {//Animacja zakończyła się
        ...
        if (!this._zamienionaWStal && this._doZamianyWStal) {
            let grafika: HTMLImageElement | null = this._animacje.mruganie.nastepnaKlatka();
            if (grafika !== null) this.grafika = grafika;
            else {//Animacja zakończyła się
     ...
     */
    public zamienWStal(): UciekajacaNagroda | 'DIAMENCIK' | null {
        //Jeżeli już została zamieniona lub już jest oznaczona do zamiany -> do widzenia
        if (this._zamienionaWStal || this._doZamianyWStal) return null;
    ...
    public aktualizujUkryte(): void {
        if (this._diamencik === null) return;
